...
namespace Nop.Services.Customers
{
    public static class CustomerExtensions
    {
        ...
        public static string GetPhone(this Customer customer)
        {
            if (customer == null)
                return "";
            var _phone = customer.GetAttribute<string>(SystemCustomerAttributeNames.Phone);
            return _phone;
        }
        ...
        /// <param name="customer">Customer</param>
        /// <returns>Customer full name</returns>
        public static string GetAddress(this Customer customer)
        {
            if (customer == null)
                return "";
            var _address = "";
            if(customer.Addresses.Count>0)
            {
                ...
                _address = adr.Address1;
                if (!string.IsNullOrEmpty(adr.Address2))
                    _address = _address + " " + adr.Address2;
                if (adr.quanhuyen != null)
                    _address = string.Format("{0}, {1}, {2}", _address, adr.quanhuyen.Ten, adr.StateProvince.Name);
                if (adr.StateProvince != null)
                    _address = string.Format("{0}, {1}", _address, adr.StateProvince.Name);
